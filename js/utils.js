...
Date.prototype.timesince = function (until) {
    ...
    if (isNaN(day_diff) || day_diff < 0 || day_diff >= 31) return this.strftime("%Y-%m-%d %H:%M:%S");

    return day_diff == 0 && (diff < 60 && "только что" || diff < 120 && "1 минуту назад" || diff < 3600 && Math.floor(diff / 60) + " минут назад" || diff < 7200 && "1 час {0} минут назад".format(Math.floor((diff - 3600 * Math.floor(diff / 3600)) / 60)) || diff < 86400 && Math.floor(diff / 3600) + " часов {0} минут назад".format(Math.floor((diff - 3600 * Math.floor(diff / 3600)) / 60))) || day_diff == 1 && "Вчера" || day_diff < 7 && day_diff + " дней назад" || day_diff < 31 && Math.ceil(day_diff / 7) + " недель назад";
};
...
function ui_confirm(html, settings, callback) {
    var buttons = [{ name: 'Да', cls: 'btn-primary', callback: callback }, { name: 'Нет', cls: 'btn-grey', callback: null }];
...
function after_submit($form, data) {
    ...
    if (typeof data == 'string') {
        ...
        if (scroll_to.length) {
            $('html, body').animate({ 'scrollTop': scroll_to.offset().top - 30 });
        }
